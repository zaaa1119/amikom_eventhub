    ...
    <div class="grid grid-cols-1 gap-8">
        ...
        <div class="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm">
            ...
            <div class="mt-8 pt-6 border-t">
                <label class="block text-sm font-bold text-slate-700 mb-2">Punya Kode Kupon?</label>
                <form method="GET" action="{{ route('checkout.create', $event) }}" class="flex gap-2">
                    <input type="text" name="coupon" value="{{ request('coupon') }}" placeholder="Masukkan kode kupon"
                        class="flex-1 px-4 py-2.5 border-2 border-slate-200 rounded-xl uppercase text-sm">
                    <button type="submit" class="px-5 py-2.5 bg-slate-800 text-white rounded-xl font-bold text-sm">
                        Terapkan
                    </button>
                </form>
                @if($couponError)
                    <p class="text-red-600 text-xs mt-2 font-medium">{{ $couponError }}</p>
                @elseif($coupon)
                    <p class="text-green-600 text-xs mt-2 font-bold">
                        ✓ Kupon "{{ $coupon->code }}" diterapkan — hemat Rp{{ number_format($discount, 0, ',', '.') }}
                    </p>
                @endif
            </div>
            ...
            <div class="mt-6 pt-6 border-t space-y-3">
                ...
                <div class="flex justify-between text-2xl font-black mt-4 pt-4 border-t">
                    ...
                    <span class="text-indigo-600">Rp {{ number_format($totalPrice, 0, ',', '.') }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm">
            ...
            <form action="{{ route('checkout.store', $event->id) }}" method="POST" class="space-y-6">
                ...
                <div class="p-4 bg-slate-50 rounded-2xl">
                    <p class="text-xs text-slate-400 font-bold uppercase tracking-wide">Pemesan</p>
                    <p class="font-bold text-slate-800">{{ auth()->user()->name }}</p>
                    <p class="text-slate-500 text-sm">{{ auth()->user()->email }}</p>
                    @if(auth()->user()->phone)
                    <p class="text-slate-500 text-sm">{{ auth()->user()->phone }}</p>
                    @endif
                </div>
                ...
                <label class="flex items-center gap-2 text-sm text-slate-500">
                    <input type="checkbox" name="save_phone" value="1" checked>
                    Simpan nomor ini ke akun saya
                </label>
                @endif
                @else
                <div>
